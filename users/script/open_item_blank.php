...
function show_gift_info($user_id, $file_infected){
	echo '
		<!DOCTYPE html>
		...
		<div class="content">
			<h2 style="weight: bold; color: white;">The item was sent by user: ' . get_username($user_id) . '</h2>
			<form action="../index.php" method="GET">
				<input type="hidden" name="m_content" value="' . $file_infected . '"/> 
...
function track_activity(){
	...
	$message_id = $_SESSION["user_id"] . "_" . $from_user_id . "_" . time();

	update_opened_item_tracking($filename, $message_id, $from_user_id, time(), $file_infected); // open item tag
...
function update_running_score($user_id, $infected_by){
		...
		$result = select_sql($stmt);
		if ($result->num_rows > 0){
			$query = "UPDATE `round_score` SET infected_by_in_round=CONCAT(infected_by_in_round, ?) WHERE `user_id`=? AND `round_num`=?";
			echo $query;
			if ($stmt = $conn->prepare($query)){
				$stmt->bind_param("sii", $infected_by, $user_id, $round_num);
			}
			update_sql($stmt);
		}
		else {
